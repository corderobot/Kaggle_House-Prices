...
dataset_K <- dataset_K[, c('TotRmsAbvGrd', 'YearBuilt', 'FireplaceQu', 'YearRemodAdd', 'GarageYrBlt', 'MasVnrArea', 'Fireplaces', 'HeatingQC', 'BsmtFinSF1', names(dataset_K)[81:94])]
#dataset_K
```
...
dataset <- dataset %>% select(id, SalePrice, everything())

dataset.clean <- dataset
```
#Particiones
```{r}
index<-createDataPartition(dataset.clean$id, p = 0.8, list=FALSE)
dataTrain<-dataset[index, ]
dataTest<-dataset[-index, ]
```

#Regresiones Lineales Cordero
```{r}
lmVar <- lm(SalePrice ~ OverallQual, data = dataTrain)
summary(lmVar)
lmVar <- lm(SalePrice ~ GrLivArea, data = dataTrain)
summary(lmVar)
lmVar <- lm(SalePrice ~ GarageCars, data = dataTrain)
summary(lmVar)
lmVar <- lm(SalePrice ~ GarageArea, data = dataTrain)
summary(lmVar)
lmVar <- lm(SalePrice ~ TotalBsmtSF, data = dataTrain)
summary(lmVar)
lmVar <- lm(SalePrice ~ X1stFlrSF, data = dataTrain)
summary(lmVar)
lmVar <- lm(SalePrice ~ FullBath, data = dataTrain)
summary(lmVar)
lmVar <- lm(SalePrice ~ ExterQual, data = dataTrain)
summary(lmVar)
lmVar <- lm(SalePrice ~ KitchenQual, data = dataTrain)
summary(lmVar)
lmVar <- lm(SalePrice ~ BsmtQual, data = dataTrain)
summary(lmVar)
lmVar <- lm(SalePrice ~ GarageFinish, data = dataTrain)
summary(lmVar)
```
###BsmtQual descartado al tener solo 2 estrellas
